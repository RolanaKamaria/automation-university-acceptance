...
import { setScoreD, setSymbol, setTitle } from "../../store/slices/rootSlice";
const Test4 = () => {
  const state = useSelector((state) => state.root);
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const [d, setD] = useState(state.score.d);
  ...
  const calculateTotal = () => {
    if (que1 && que2 && que3 && que4 && que5) {
      setWrong(false);
      dispatch(setScoreD(d));
      navigate("/test1/test2/test3/test4/test5");
    } else {
      setWrong(true);
    }
  };
  const handleBack = () => {
    navigate("/test1/test2/test3");
  };
  useLayoutEffect(() => {
    ...
    <div className="main-test1 mainbg" dir="rtl">
      ...
      <div className="test1-container">
        ...
        <div className="test-content">
          <div className="form-container test4">
            <div className="choose-form test4-med">
              <div className="choose-label">نعم</div>
              <div className="choose-label">كلّا</div>
              <div className="choose-label">ربّما</div>
            </div>
            <div className="question-container test4-ques">
              <div className="questions">
                {questions
                  .filter((ques) => ques.q === "d")
                  .map((ques) => (
              ...
              <div className="chooses test4-ch">
                {questions
                  .filter((ques) => ques.q === "d")
                  .map((ques) => (
