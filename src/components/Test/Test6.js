...
import { setScoreF, setSymbol, setTitle } from "../../store/slices/rootSlice";
const Test6 = () => {
  const state = useSelector((state) => state.root);
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const [f, setF] = useState(state.score.f);
  ...
  const calculateTotal = () => {
    if (que1 && que2 && que3 && que4 && que5) {
      setWrong(false);
      dispatch(setScoreF(f));
      navigate("/test1/test2/test3/test4/test5/test6/test7");
    } else {
      setWrong(true);
    }
  };
        ...
        <div className="test-content">
          <div className="form-container">
            ...
            <div className="question-container">
              <div className="questions test6-ques">
                {questions
                  .filter((ques) => ques.q === "f")
                  .map((ques) => (
